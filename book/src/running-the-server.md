...
Prompt enhancement happens inside the `trueblocks-dalle` library, which picks its model from `TB_DALLE_ENHANCEMENT_MODEL` and sets its own timeout; the server makes no enhancement call of its own.
